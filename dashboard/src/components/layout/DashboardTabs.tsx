...
export function DashboardTabs({
  ...
  return (
    <nav className="mb-4 sm:mb-5" aria-label="Secciones del dashboard">
      <div className="-mx-3 flex gap-2 overflow-x-auto px-3 pb-1 sm:mx-0 sm:px-0 lg:grid lg:grid-cols-5 xl:grid-cols-10">
        {tabs.map((item) => (
          <button
            key={item.id}
            type="button"
            aria-current={activeTab === item.id ? 'page' : undefined}
            className={`flex h-11 min-w-[8.75rem] shrink-0 items-center justify-center gap-2 rounded-tremor-default border px-3 text-xs font-semibold leading-tight transition lg:min-w-0 xl:text-[0.82rem] ${
              activeTab === item.id
